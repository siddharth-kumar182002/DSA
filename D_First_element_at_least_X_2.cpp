...
long long query(long long x,long long idx,long long i,long long l,long long r){
...
if(r<idx){
 ...
 if(seg[2*i+1]>=x){
     q1=query(x,idx,2*i+1,l,mid);
    
 }
 if(q1==-1){
 return query(x,idx,2*i+2,mid+1,r);
 }
 else{
    return q1;
 }
